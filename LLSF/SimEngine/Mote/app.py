    ...
    def _generate_packet(
        ...
        dataPacket = {
            ...
            'app': {
                ...
                'gen_mac': self.mote.get_mac_addr(),
            }

        }

        # update appcounter
        print "##ID",self.mote.id,"Counter", self.engine.getAsn(),self.engine.getAsn()%self.settings.tsch_slotframeLength
        self.appcounter += 1
...
class AppPeriodic(AppBase):
    ...
    def _schedule_transmission(self):
        ...
        if self.sending_first_packet:
            ...
            current_timeslot= self.engine.getAsn()%self.settings.tsch_slotframeLength
            if current_timeslot<= self.mote.gen_timeslot:
                delay = (self.mote.gen_timeslot -current_timeslot+1)*self.settings.tsch_slotDuration
            else:
                delay = (self.settings.tsch_slotframeLength+self.mote.gen_timeslot - current_timeslot+1) * self.settings.tsch_slotDuration
			...
			# set steady period of time :)
            delay = self.settings.app_pkPeriod + self.settings.app_pkPeriod * 1 * self.settings.tsch_slotDuration  #:)

        gen_timeslot= int( ((self.engine.getAsn() + delay / 0.01) % self.settings.tsch_slotframeLength - 1))
        print "#ID",self.mote.id,"Gen Timeslot", gen_timeslot

        self.mote.gen_timeslot=gen_timeslot
