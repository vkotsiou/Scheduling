...
class Tsch(object):
    ...
    def __init__(self, mote):
        ...
        self.add_slotframe(
            slotframe_handle = 0,
            length           = 101 # self.settings.tsch_slotframeLength #:)
        )
    ...
    def get_first_packet_to_send(self, dst_mac_addr=None):
        ...
        if dst_mac_addr is None:
            ...
            for packet in self.txQueue:
                ...
                abort = 0
                if packet['type'] == 'DATA':
                    cell = self.mote.tsch.slotframes[1].slots[asn % self.settings.tsch_slotframeLength]

                    if len(cell) != 0:
                        if len(cell[0].options) > 1:  # autonomouus or shared
                            abort = 1

                #abort=0 #:) disable filtering  -- No Filtering
                if packet['mac']['dstMac'] == dst_mac_addr and abort == 0:
        ...
        if packet_to_send != None: #:)
            print "&ID", self.mote.id, "SND","Type", packet_to_send['type'],  "ASN", self.engine.getAsn(),"Timeslot", self.mote.engine.getAsn() % self.settings.tsch_slotframeLength,  # "Next",next
            if packet_to_send['type'] == 'DATA':
                print "Src", packet['app']['gen_mac'], "Counter", packet['app']['appcounter']

                print "Queue -"
                for p in self.txQueue:
                    if p['type']=='DATA':
                        print "\tSrc", p['app']['gen_mac'],"counter", p['app']['appcounter'],"-",p['app']['timestamp'],"Delay",self.engine.getAsn()- p['app']['timestamp']

                print "\nSchedule - TX",
                for cell in self.mote.tsch.slotframes[1].slots:
                    if len(cell) != 0 and len(cell[0].options) == 1 and cell[0].options[0] == 'TX':
                        print cell[0].slot_offset,",",
                print "\n"
    ...
    def rxDone(self, packet, channel):
        ...
        if packet != None:              #:)
            if packet['type'] == 'DATA' and packet['mac']['dstMac']== self.mote.get_mac_addr():
                 print "&ID RECV", self.mote.id, "Asn", self.engine.getAsn(),"Timeslot",self.mote.engine.getAsn()%self.settings.tsch_slotframeLength,"Src",packet['app']['gen_mac'],"Counter",packet['app']['appcounter'],"Delay", -packet['app']['timestamp'] +self.engine.getAsn()
    ...
    def _action_active_cell(self):
        ...
        # :) debug purposes
        if asn / self.settings.tsch_slotframeLength == self.settings.exec_numSlotframesPerRun - 1 and asn % self.settings.tsch_slotframeLength == 0:
            print "~ID", self.mote.id, "Schedule"
            for cell in self.mote.tsch.slotframes[0].slots:
                if len(cell) != 0:
                    print cell
            for cell in self.mote.tsch.slotframes[1].slots:
                if len(cell) != 0:
            ...
            for i in range(0, len(self.mote.tsch.slotframes)):
                for cell in self.mote.tsch.slotframes[i].slots:
                    ...
                    if len(cell) != 0:
                        ...
                        #print cell[0].slot_offset, cell[0].options
            #print shared ,tx,rx
