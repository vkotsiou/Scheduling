...
class Rpl(object):
    ...
    def indicate_preferred_parent_change(self, old_preferred, new_preferred):
        ...
        if new_preferred is None:
            ...
            self.trickle_timer.reset()
        print "*ID", self.mote.id, "ASN", self.engine.getAsn(), "Parent Change", old_preferred, "-->", new_preferred  #:)
...
class RplOF0(RplOFBase):
    ...
    def update(self, dio):
        ...
        # Calculate mote hops using DIO messages.  :)
        hops = dio['net']['hops']
    ...
    def _update_preferred_parent(self):
        ...
        if (
            ...
            if new_parent is None:
                ...
                #calculate limits
                self.mote.sf.Cmin, self.mote.sf.Cmax = self.mote.sf.limits()

                # initiate 6P add command
                ...
                print "*ID", self.mote.id, "Limits", "\n", self.mote.sf.Cmin, self.mote.sf.Cmax

            print "*ID", self.mote.id, "Hops", self.mote.hops
            self.rpl.indicate_preferred_parent_change(
                old_preferred = old_parent_mac_addr,
