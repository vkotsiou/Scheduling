...
class AppPeriodic(AppBase):
    ...
    def _send_a_single_packet(self):
        ...
        if self.mote.lock_schedule==0:  #:)
            ...
            if len(self.mote.tsch.txQueue)==self.mote.tsch.txQueueSize:
                print "?ID",self.mote.id, "Queue FULL"
                for i in range(0, len(self.mote.tsch.txQueue) ):
                    packet_to_drop = self.txQueue[-1]
                    self.mote.tsch.dequeue(packet_drop)
            self.sending_first_packet==True
