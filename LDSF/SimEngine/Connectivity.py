        ...
        for channel in d.TSCH_HOPPING_SEQUENCE[:self.num_channels]:
            ...
            for listener_id in self._get_listener_id_list(channel):
                ...
                for t in alltransmissions:
                    ...
                    #pdr=1.0 #:) ideal network
                    #print "@@",pdr,random_value
        ...
        for target_mote_id in self.mote_id_list:
            ...
            while mote_is_deployed is False:
                ...
                if (
                    ...
                    continue
        #print square_side :)
        print "Cord", self.coordinates
        #print "Connectivity",self._matrix
        print "Neighborhood"

        for mote1 in self.engine.motes:
           for mote2 in self.engine.motes:
                if self._matrix[mote1.id][mote2.id][26]['pdr'] >= 0.0:
                    print mote1.id, "-->", mote2.id, self._matrix[mote1.id][mote2.id][26][
                        'pdr'], "|", self._distance(mote1.id, mote2.id)


    def _distance(self,mote1,mote2):
        (x1,y1)=self.coordinates[mote1]
        (x2, y2) = self.coordinates[mote2]
