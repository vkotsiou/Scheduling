...
class Rpl(object):
    ...
    def changePreferredParent(self):
        # FIXME: when we implement IPv6 address or MAC address, we should
        # define the return type of this method. Currently, this method can
        # return a node ID, a MAC address, or an IPv6 address since they are
        # all the same value for a certain mote.
        return self.of.change_preferred_parent()
    # admin
    ...
    def indicate_preferred_parent_change(self, old_preferred, new_preferred):
        ...
        print "&ID", self.mote.id, "Parent Change", old_preferred, "-->", new_preferred  #:)
...
class RplOFBase(object):
    ...
    def change_preferred_parent(self):
        pass
...
class RplOF0(RplOFBase):
    ...
    def parents(self):   #:)
        ...
        for neighbor in self.neighbors:
            if self._calculate_rank(neighbor) is None:
                # skip this one
                continue
            if neighbor['mac_addr'] in self.mote.black_parent: # :) skip black parents - full of childs.
                #skip black parent
                continue
            if (
                    (self.rank is None)
    ...
    def update(self, dio):
        ...
        if neighbor is None:
            ...
            neighbor['hops'] = hops #:) edw change parent code
        else:
            old_hops = neighbor['hops']
            neighbor['hops'] = hops

            if old_hops != hops and neighbor['mac_addr'] == self.preferred_parent['mac_addr']:
                self.mote.hops = self.preferred_parent['hops'] + 1
                print "*ID", self.mote.id, "Hops #", self.mote.hops, "|", self.preferred_parent['mac_addr']
    ...
    def update_etx(self, cell, mac_addr, isACKed):
        ...
        if isACKed is True:
            neighbor['numTxAck'] += 1
        #print "@ID",self.mote.id,"Pkts",neighbor['numTx'],"Acks",neighbor['numTxAck']
    ...
    def _update_preferred_parent(self):
        ...
        if (
                ...
                # :) here change the code to disable parent change
                #and
        ...
        if (
            ...
            if new_parent is None:
                ...
                self.mote.hops = self.preferred_parent['hops'] + 1  #:)
                print "*ID", self.mote.id, "Hops", self.mote.hops, "|", old_parent_mac_addr, "-->", self.preferred_parent[
                    'mac_addr']
            self.rpl.indicate_preferred_parent_change(
                old_preferred = old_parent_mac_addr,
            ...
            # do nothing
            pass
    def change_preferred_parent(self):    # :) change prefered father in case of full of childs
        print "Change Preferred Father",self.mote.id
        '''
        if (
                (self.preferred_parent is not None)
                # :) here change the code to disable parent change
...
class RplOFBestLinkPDR(RplOF0):
    ...
    def _update_preferred_parent(self):  #:) here
        if self.parents:
