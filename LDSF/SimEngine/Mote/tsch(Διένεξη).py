...
class Tsch(object):
    ...
    def __init__(self, mote):
        ...
        self.activate =-1 # start -> -1, Deactivate -> 0, Activate -> 0
        self.prev_activate=-1
    ...
    def add_vCell(self, index, mac_addr, no, state, options, flow_id=1):
    ...
    def add_flow_gen(self, vtimeslot, id):  # dont estimate first CELL
        ...
        if self.gen_lock <=1:
            ...
            i = 0
            extra1=0
            while (i <2*d.TSCH_MAXTXRETRIES + extra):
                extra1 += self.add_vCell(vtimeslot + i * 2 * d.SubFrameSize, self.mote.get_mac_addr(), i, 1, 'TX',id)
                i += 1
                if extra > 10:
                    break
        ...
        l.sort()
        if id==1:
            print "@ID DATA", extra, self.mote.id, "VSlotFrame Gen", l
        else:
    ...
    def add_flow_rx(self, vtimeslot, mac_addr, hops,id):
        ...
        i = 0
        extra1=0
        while (i < (hops + 1  +1) * d.TSCH_MAXTXRETRIES + extra):
            extra1 += self.add_vCell(vtimeslot + i * 2 * d.SubFrameSize, mac_addr, i, 1, 'RX',id)
            i += 1
            if extra > 10:
                break
        ...
        l = list(self.VirtualSlotFrame)
        l.sort()
        print "@ID", self.mote.id,"Extra", extra,"VSlotFrame RX", l

    def add_flow_tx(self, vtimeslot, mac_addr, hops,id):
        extra = 0
        i = 0
        extra1=0
        while (i < (hops + 2+1) * d.TSCH_MAXTXRETRIES + extra):
            extra1 += self.add_vCell(vtimeslot + i * 2 * d.SubFrameSize, mac_addr, i, 1, 'TX',id)
            i += 1
            if extra > 10:
                break
        ...
        l = list(self.VirtualSlotFrame)
        l.sort()
        print "@ID", self.mote.id,"Extra",extra, "VSlotFrame TX", l
    ...
    def get_first_packet_to_send(self, dst_mac_addr=None):
        ...
        if dst_mac_addr is None:
            ...
            if vindex in self.VirtualSlotFrame:
                ...
                if len(self.txQueue)!=0:
                    print "%ID", self.mote.id, "Virtual", self.VirtualSlotFrame[vindex],dataSND,dataDAO,dataKeepAlive
                    print "%ID", self.mote.id, vindex, len(self.txQueue),"Queue", self.txQueue
                    print "%ID", self.mote.id,dst_mac_addr,data_mac
            ...
            for packet in self.txQueue:
                if dataSND == True:
                    ...
                    if packet['mac']['dstMac'] == dst_mac_addr and packet['type'] == 'DAO':
                        # return the first one having the dstMac
                        packet_to_send = packet
                        next=1
                        break
                elif dataKeepAlive == True :
                    if packet['mac']['dstMac'] == dst_mac_addr and packet['type'] == 'KEEP_ALIVE':
                        # return the first one having the dstMac
                        packet_to_send = packet
                        next=1
                        break
            ...
            if packet_to_send!=None:
                print "%ID",self.mote.id,"Type",packet_to_send['type'],"Vindex",vindex,"ASN",self.engine.getAsn(), #"Next",next
    ...
    def txDone(self, isACKed, channel):
        ...
        if self.pktToSend['mac']['dstMac'] == d.BROADCAST_ADDRESS:
            ...
            if isACKed:
                # ... which was ACKed
                #:) deactivate TX
                if self.pktToSend['type'] == 'DATA' and vindex in self.VirtualSlotFrame:
    ...
    def _action_active_cell(self):
        ...
        vindex = self.asn2vindex(asn)
        #print "`ID",self.mote.id,"ASN",asn,"->",vindex,"Timeslot",asn%101
        # , "-",len(self.flows.keys()),"=",len(self.under_flows), "Flows",self.flows,"Under",self.under_flows

        '''
        if self.activate==-1:
            self.activate=0

        if self.mote.is_leaf == 1:
            if vindex in self.VirtualSlotFrame:
                self.activate=1
        ...
        if self.activate!=self.prev_activate:
            #print "@",  self.prev_activate,"-->",self.activate,
            print "\n |ID",self.mote.id,"ASN",asn,"ACTIVE",self.activate
            s1=":ID "+str(self.mote.id)+" ASN "+str(asn)+" ACTIVE "+str(self.activate)+"\n"
            self.fd.write(s1)
            print "\n"

        self.prev_activate=self.activate
        '''
        ...
        if self.mote.is_leaf == 1:#>=10
            if vindex != -1 and vindex not in self.VirtualSlotFrame and len(self.VirtualSlotFrame) != 0 and vindex % 100 >= 10  or ( vindex in self.VirtualSlotFrame and self.is_active_vslot(asn) == 0):
                abort = 1
        elif vindex != -1 and vindex not in self.VirtualSlotFrame and len(self.VirtualSlotFrame) != 0 and vindex % 100 >= 10  or ( vindex in self.VirtualSlotFrame and self.is_active_vslot(asn) == 0):
            abort = 1
        ...
        if abort == 1 :#and 0==1:
            #print "@ID",self.mote.id,"abort",vindex
            self._schedule_next_active_slot()
            return
            #pass
